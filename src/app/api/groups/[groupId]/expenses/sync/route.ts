...
function buildParticipantIdMapByName(
  ...
  for (const participant of localParticipants) {
    const remoteId = remoteByName.get(
      normalizeParticipantName(participant.name),
    )
    if (remoteId) {
...
function buildSyncLines(
  ...
  conflictDifferencesByRemoteIndex: Map<
    number,
    Array<'category' | 'splitMode' | 'paidFor'>
  >,
): SyncLine[] {
  ...
  for (const remoteExpense of remoteExpenses) {
    ...
    if (baseMatches.length > 0) {
      ...
      syncLines.push({
        ...
        differences:
          conflictDifferencesByRemoteIndex.get(remoteExpense.index) ?? [],
...
export async function POST(
  ...
  try {
    ...
    const selectedLineMap = new Map(
      selectedLines.map((line) => [line.lineId, line.direction]),
    )
    ...
    for (const [lineId, direction] of selectedLineMap.entries()) {
      ...
      switch (action) {
        ...
        case 'delete_local': {
          ...
          if (line.localExpenseId)
            deleteLocalExpenseIds.add(line.localExpenseId)
          break
        }
        case 'create_remote': {
          if (line.entity === 'participant' && line.localExpenseId) {
          ...
          if (line.localExpenseId)
            createRemoteExpenseIds.add(line.localExpenseId)
          break
        }
        case 'update_remote': {
        ...
        case 'delete_remote': {
          ...
          if (line.remoteExpenseId)
            deleteRemoteExpenseIds.add(line.remoteExpenseId)
          break
        }
      }
    }
    ...
    if (createLocalParticipantIds.size > 0) {
      const remoteParticipantsById = new Map(
        remoteJSON.participants.map((participant) => [
          participant.id,
          participant,
        ]),
      )
      ...
      for (const remoteParticipantId of createLocalParticipantIds) {
        const remoteParticipant =
          remoteParticipantsById.get(remoteParticipantId)
    ...
    if (importRemoteIndexes.size > 0) {
      ...
      const filteredRemoteExpenses = remoteJSON.expenses.filter(
        (expense, index) => {
    ...
    if (hasRemoteActions) {
      ...
      const createRemoteParticipantNames = Array.from(
        createRemoteParticipantIds,
      )
