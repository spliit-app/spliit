 ...
 * one or several, comma-separated. Add yours here, then add the matching entry
 * in `./registry` and the matching branch in `./config` — both are
 * compiler-enforced, so `npm run check-types` will point at whatever is
 * missing.
 *
 ...
 */
export const ANALYTICS_PROVIDER_IDS = ['console', 'plausible', 'umami'] as const
